...
for (marker_group in names(markers_list)) {
  for (marker_name in markers_list[[marker_group]]){
    ...
    cat(sprintf("#### %s (%s) - %s\n", marker_name, marker_id, marker_group))
    plot_data <- reducedDim(sce, "UMAP") %>%
      as_tibble() %>%
      mutate(
    ...
    gg_scatter <- ggplot(plot_data) +
      ...
      theme_bw()
    nonzero_quantile <- plot_data %>% filter(logcounts > 0) %>% pull(logcounts) %>% quantile(0.01)
    n_cells_over_quantile <- plot_data %>% filter(logcounts > nonzero_quantile) %>% nrow()
    gg_histogram <- ggplot() +
      geom_histogram(aes(x = logcounts), data = plot_data, colour = "black", fill = "grey", binwidth = 0.1) +
      geom_vline(xintercept = nonzero_quantile) +
      labs(
        title = sprintf("%s | %s (%s)", marker_id, marker_name, marker_group),
        subtitle = sprintf("%s, (%s cells positive)", sample_name, format(n_cells_over_quantile, big.mark = ","))
      ) +
      theme_bw()
