...
### Clusters vs Sex (predicted)

Male predicted as expression of roX2 higher than the 1% quantile of log-normalised non-zero count values.

```{r cluster_vs_roX2_sex}
roX2_cutoff <- tibble(
  logcounts = assay(sce, "logcounts")["FBgn0019660",]
  ) %>%
  filter(logcounts > 0) %>%
  pull(logcounts) %>%
  quantile(0.01)
sce$roX2_sex <- factor(
  x = ifelse(
    test = assay(sce, "logcounts")["FBgn0019660",] >= roX2_cutoff,
    yes = "male",
    no = "female"
  ),
...
saveRDS(sce, gsub(".html$", ".rds", snakemake@output[[1]]))
```