...
markers_list <- list(
  ...
  "Glutamatergic" = c("VGlut", "Gs2"),
  "GABAergic" = c("Gad1", "VGAT"),
  "Dopaminergic" = c("ple", "Ddc"),
  "Serotonergic" = c("SerT"), # "Trhn"?
  "Octopaminergic / Tyraminergic" = c("Tbh", "Tdc2")
)
```

```{r, message=FALSE, results='asis', fig.height=6, fig.width=8}
# plot_list <- list()
for (cell_type in names(markers_list)) {
  cat("### ", cell_type, "\n")
  for (cell_marker_name in markers_list[[cell_type]]){
    message(cell_marker_name)
    cell_marker_id <- gtf_gene_data %>%
      filter(gene_name == cell_marker_name) %>%
      pull(gene_id)
    stopifnot(length(cell_marker_id) == 1L)
    ...
    gg_plot <- ggplot(plot_data) +
      ...
      theme_bw()
    print(gg_plot)
    # plot_list[[cell_marker_name]] <- gg_plot
  }
}
```
