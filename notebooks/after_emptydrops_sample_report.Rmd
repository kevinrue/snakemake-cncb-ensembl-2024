...
```{r markers_plots, echo=FALSE, fig.height=10, fig.width=12}
  ...
  for (cell_marker_id in markers_list[[cell_type]]){
    ...
    gg_violin <- ggplot() +
      ...
      labs(
        x ="Cluster",
        y = "Log-counts",
        title = paste0(cell_marker_id, " | ", marker_gene_name, " (", cell_type, ")"),
        subtitle = sample_name
      ) +
      theme_bw() +
      theme(
    ...
    cowplot_row1 <- cowplot::plot_grid(gg_scatter, gg_histogram, nrow = 1)
    cowplot_row2 <- gg_violin
    cowplot_all <- cowplot::plot_grid(cowplot_row1, cowplot_row2, nrow = 2)
    print(cowplot_all)
  }
}
```
