...
df <- matrix(
  data = c(
    format_bold("Sample name"), fs::path_ext_remove(basename(rds_file))
  ),
  ncol = 2, byrow = TRUE
)
knitr::kable(x = df)
...
```{r filter_stats_plot, echo=FALSE}
ggplot_data <- emptydrops_results %>% 
  as.data.frame() %>% 
  as_tibble() %>% 
  mutate(
...
ggplot() +
  geom_line(aes(rank, FDR), ggplot_data) +
  geom_label(aes(barcodes, threshold, label = format(barcodes, big.mark = ",")), filter_stats) +
