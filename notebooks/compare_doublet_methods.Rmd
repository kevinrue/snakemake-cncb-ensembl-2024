...
seu_pann_colname <- grep("pANN_", colnames(seu_dbl[[]]), value = TRUE)
comparison_table <- tibble(
  barcode = barcodes_union,
  scdblfinder_class = colData(sce_dbl)[barcodes_union, "class"],
  doubletfinder_class = seu_dbl[[]][barcodes_union, seu_class_colname],
  scdblfinder_score = colData(sce_dbl)[barcodes_union, "score"],
  doubletfinder_pANN = seu_dbl[[]][barcodes_union, seu_pann_colname]
) %>% 
  bind_cols(
    reducedDim(sce_umap, "UMAP")[barcodes_union, ]
  ) %>% 
  mutate(
    doublet = (scdblfinder_class == "doublet") + 2 * (doubletfinder_class == "Doublet"),
    doublet = recode(
      doublet,
      `0` = "singlet",
      `1` = "scdblfinder",
...
```{r}
table(
  scdblfinder = comparison_table$scdblfinder_class,
  doubletfinder = comparison_table$doubletfinder_class
)
```
