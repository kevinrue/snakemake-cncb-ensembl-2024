...
message("GRanges object size: ", format(object.size(gtf_data), unit = "GB"))
...
message("Duplicated gene boundaries:", format(nrow(duplicated_gene_boundaries), big.mark = ","))
...
for (i in seq_len(nrow(duplicated_gene_boundaries))) {
  # for (i in 1) {
  for_seqnames <- duplicated_gene_boundaries %>% filter(row_number() == i) %>% pull(seqnames)
  for_start <- duplicated_gene_boundaries %>% filter(row_number() == i) %>% pull(start)
  for_end <- duplicated_gene_boundaries %>% filter(row_number() == i) %>% pull(end)
  for_strand <- duplicated_gene_boundaries %>% filter(row_number() == i) %>% pull(strand)
  for_dup_gene_info <- all_genes %>% filter(
